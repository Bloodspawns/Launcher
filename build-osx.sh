...
java -jar packr_${PACKR_VERSION}.jar \
    ...
    Djna.nosys=true \
    --output \
    native-osx/BlueLite.app

cp target/filtered-resources/Info.plist native-osx/BlueLite.app/Contents

echo Setting world execute permissions on BlueLite
pushd native-osx/BlueLite.app
chmod g+x,o+x Contents/MacOS/BlueLite
popd

codesign -f -s "${SIGNING_IDENTITY}" --entitlements osx/signing.entitlements --options runtime native-osx/BlueLite.app || true

# create-dmg exits with an error code due to no code signing, but is still okay
# note we use Adam-/create-dmg as upstream does not support UDBZ
./node_modules/.bin/create-dmg native-osx/BlueLite.app native-osx/ || true

mv native-osx/BlueLite\ *.dmg native-osx/BlueLite.dmg
